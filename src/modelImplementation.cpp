...
ModelImplementation::ModelImplementation(ModelImplementation &obj)
{
    if (&obj == this)
        return;
...
ModelImplementation::~ModelImplementation()
{
    Model::vectorFlow itf = flowBegin();
    for (itf; itf != flowEnd(); itf++)
        delete *itf;

    Model::vectorSystem its = systemBegin();
...
{
    cout << endl;
    cout << "Sub systems and their value:" << endl;
...
{
    System *s = new SystemHandle(id, initValue);
